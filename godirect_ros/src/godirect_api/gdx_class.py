...
class gdx():
    ...
    if isinstance(device_to_open, str): 
      for device in found_devices: 
        if device_to_open == str(device.name):
          ...
          raise Exception(f'Device {device_to_open} not found in {found_devices}')
    else:
  ...
  def start(self, sampling_rate):
    ...
    rospy.loginfo(f'Starting data collection.')
  ...
  def calibrate_sensor(self, seconds=0.5):
    ...
    rospy.loginfo(f'Calibration successful! Sensor offset: {self.sensor_offset:.5f}')
    return
    
#  def read(self, publisher, measurement_type='grip'):
  def read(self, publisher, measurement_type='grip', param_a=1.0876 , param_b=0.2633):             
    ...
    # Iterator
    sig_iter = 0
    rospy.loginfo(f'Publishing grip data. Press ctrl-c to stop ...')
    try:
      while not rospy.is_shutdown(): 
        ...
        if self.device_hn.read():  
          ...
          for ts, id, value in zip(timestamps_iter, sig_id, data):
            ...
            force_data.grip_force = ((param_a * value) - param_b) - sensor_offset
